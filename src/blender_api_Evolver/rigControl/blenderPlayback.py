...
#register new props for storing rate values of expression, as rated by the user, during Genetic algorithm based new expression generation
...
class BLPlayback(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
           ...
           # update NLA based GA related gestures 
            gestures = eva.gesturesList_IGA[:]  	# prevent in-situ removal while iterating bug
            for gesture in gestures:
                # Leave strip time for backward compatable with blender with version lower 2.75
                gesture.stripRef.strip_time += gesture.speed * timeScale
                gesture.strip_time_kfp.co[1] += gesture.speed * timeScale
                if gesture.stripRef.strip_time > gesture.duration:
                    if gesture.repeat > 1:
                        ...
                        eva._deleteGesture_IGA(gesture)
...
# GA related
            if bpy.context.scene['keepAlive']:
                ...
                # Ensure the strip is looping and sync strip props with cycle props.
                for cycle in eva.cyclesSet:
                    if cycle.name not in [gesture.name for gesture in eva.gesturesList_IGA]:
                        # Strip finished, create new one
                        eva.newGesture_IGA(cycle.name, repeat=10, speed=cycle.rate, magnitude=cycle.magnitude)
                    else:
                        # Update strip properties
                        for gesture in eva.gesturesList_IGA:
                            if gesture.name == cycle.name:
                                gesture.stripRef.influence = cycle.magnitude
                                gesture.speed = cycle.rate
                                gesture.stripRef.mute = False
