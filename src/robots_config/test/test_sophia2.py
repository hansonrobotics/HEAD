...
from testing_tools.misc import MessageQueue, get_rosbag_file, rosbag_msg_generator, PololuSerialReader
...
class SophiaTest2(unittest.TestCase):
    ...
    def test_eyes_pau_message(self):
        reader = PololuSerialReader('%s/pololu1' % CWD)
        self.runner.config.nodes = filter(
            lambda node: node.name != 'blender_api', self.runner.config.nodes)
        self.runner.launch()
        ...
        eyes_pau_motors = set(rosparam.get_param('/sophia/pau2motors/topics')['eyes_pau'].split(';'))
        dynamixel_motors = {'Frown_L', 'Frown_R', 'Smile_L', 'Smile_R', 'Jaw_Up_Down'}
        eyes_pau_motors = eyes_pau_motors - dynamixel_motors
        motors = rosparam.get_param('/sophia/motors')
        pololu_motor_config = {}
        for motor in motors:
            if 'motor_id' in motor:
        ...
        self.assertTrue(len(eyes_pau_motors)>0)

        eyes_pau_messages = MessageQueue()
        eyes_pau_messages.subscribe('/sophia/eyes_pau', pau)
        ...
        pau_message = pau()
        pub, msg_class = rostopic.create_publisher('/sophia/eyes_pau', 'pau2motors/pau', True)
        pub.publish(msg_class(m_coeffs=[0]*48))
        time.sleep(2)

        print 'no pau %s' % eyes_pau_messages.queue.qsize()
        for _ in range(len(eyes_pau_motors)*3):
            motor_id, cmd, value = reader.read()
            config = pololu_motor_config[motor_id]
            if cmd == 'position':
                print motor_id, cmd, value
        ...
        # every motor has gotten a command message
        self.assertEqual((eyes_pau_motors-set(counter.keys())), set([]))
