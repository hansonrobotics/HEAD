...
class Safety():
    ...
    def rule_timing(self, m, r):
        ...
        if rule['started']:
            ...
            if rule['started'] + rule['t1'] + rule['t2'] > time.time():
                limit += (1 - limit) * (rule['started'] + rule['t1'] + rule['t2'] - time.time()) / rule['t2']
            # Period increased for extreme position to be available
            if rule['started'] + rule['t1'] + rule['t2'] + rule['t3'] < time.time():
                limit += (1 - limit) * (time.time() - rule['started'] - rule['t1'] - rule['t2'] - rule['t3']) / rule[
                    't4']
    ...
    def rule_loading(self, m, r):
        rule = self.rules[m][r]
        direction = 1 if rule['direction'] == 'max' else -1
        extreme = self.motor_loads[rule['motor_id']] * direction > rule['extreme']
        if rule['started']:
    ...
    def set_motor_relative_pos(self, m, pos, direction):
        v = self.get_abs_pos(m, direction, pos)
    ...
    def correction_cb(self, msg):
        if msg.joint_name in self.corrections.keys():
